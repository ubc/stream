...
class Carry_Update_Post {
	...
	function push_updated_post( $post_id, $post ) {
		...
		// Don't update if the post is password protected or if it isn't a post.
		...
		<article id="post-{{=it.id}}" class="post-{{=it.id}} post type-post status-publish hentry ">
			<header class="entry-header">
				<h1 class="entry-title">
					<a href="{{=it.permalink}}" title="Permalink to {{=it.title}}" rel="bookmark">{{=it.title}}</a>
				</h1>
				<div class="entry-meta">
					Posted on
					<a href="{{=it.permalink}}" title="{{=it.permalink}}" rel="bookmark">
						<time class="entry-date" datetime="{{=it.gmt_date}}" pubdate>{{=it.date}}</time>
					</a>
					<span class="byline"> by
						<span class="author vcard">
							<a class="url fn n" href="{{=it.author_url}}" title="View all posts by {{=it.author}}" rel="author">{{=it.author}}</a>
						</span>
					</span>
				</div><!-- .entry-meta -->
			</header><!-- .entry-header -->
			...
			<footer class="entry-meta">
				...
				<a href="{{=it.permalink}}" title="Permalink to {{=it.title}}" rel="bookmark">permalink</a>
				<span class="sep"> | </span> 
				<span class="comments-link">
					<a href="{{=it.permalink}}#respond" title="Comment on hey there">Leave a comment</a>
				</span>
			</footer>
		</article>
		<?php
